...
ensemble_TEB_exp0 = zeros(1, length(ensemble_R));
...
%% Echantilloneur : Démoduleur bande de base 
    ...
    ensemble_TEB_exp0(i) = Taux_erreur;
    i = i + 1;
end 
...
% Choix du rapport signal à bruit par bit souhaité à l entrée du récepteur
ensemble_R = linspace(0.001, 6, 10);
ensemble_TEB_exp40 = zeros(1, length(ensemble_R));
...
%% Echantilloneur : Démoduleur bande de base 
    ...
    erreur = (reception == bits);
    Taux_erreur = 1-mean(erreur);
    ensemble_TEB_exp40(i) = Taux_erreur;
    i = i + 1;
end 
...
figure;
semilogy(ensemble_R, ensemble_TEB_exp0,'LineWidth',2);
hold on
semilogy(ensemble_R, ensemble_TEB_exp40,'LineWidth',2);
...
legend("TEB experimental avec une erreur de phase de 100° corrigée", "TEB experimental avec une erreur de phase de 100° non corrigée");
